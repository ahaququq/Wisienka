...
import io.github.ahaququq.imkotlin.*
...
class LoginScreen(var parent: Screen? = null) : Screen(Text.translatable("wisienka.gui.login.title")) {
	...
	var passwordsDontMatch: Boolean = false
	val username: ImString = ImString()
	val password: ImString = ImString()
	val password2: ImString = ImString()
				...
				tabBar("MainOptions") {
					...
					tabItem("Register") {
						...
						table("UsernamePassword", 2) {
							...
							column {
								if (fullWidth.inputText("##password2", password2, InputTextFlags.Password, InputTextFlags.EnterReturnsTrue)) {
									register()
								}
							}
						}
	...
	private fun premiumLogin() {
		Wisienka.logger.info("Premium login")
		LoginHelper.premiumLogin()
	}

	private fun register() {
		Wisienka.logger.info("Registration")
		passwordsDontMatch = !(password.get().equals(password2.get(), false))
		LoginHelper.register(username, password, closable)
	}

	private fun passwordLogin() {
		Wisienka.logger.info("Password login")
		LoginHelper.passwordLogin(username, password)
	}

	fun setStyle() {
		val colors = ImKotlin.style.colors
